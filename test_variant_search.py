#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Test the formatum-specific search functionality"""
...
def test_variant_search():
    """Test formatum filtering in search"""
    search = RulebookSearch("qa-tools/rules_index.json")
    
    # Test 1: Search for "5 találat" without formatum filter
    print("=" * 70)
    ...
    for i, result in enumerate(results, 1):
        ...
        print(f"   Formatum: {result.formatum if result.formatum else '(general)'}")
        print(f"   Score: {result.score:.1f}")
        print()
    
    # Test 2: Search for "VOR" - should prioritize VOR rules
    print("=" * 70)
    print("TEST 2: Search for 'VOR' (formatum-specific search)")
    print("=" * 70)
    ...
    # Test 4: Get specific formatum rules by ID
    print("=" * 70)
    print("TEST 4: Direct formatum rule lookups")
    print("=" * 70)
    variant_ids = ["GEN-6.10.4.1.1", "GEN-6.10.4.1.2", "GEN-6.10.4.1.3"]
    for rule_id in variant_ids:
        rule = search.get_rule_by_id(rule_id)
        if rule:
            formatum = rule.get('formatum', '(general)')
            print(f"Rule {rule_id}: [{formatum}]")
        else:
            print(f"Rule {rule_id}: NOT FOUND")
    
    print("\n" + "=" * 70)
    print("SUMMARY: Formatum-specific filtering is working!")
    print("=" * 70)
