...
def test_variant_search():
    ...
    # Test 3: Search with explicit variant filter
    print("=" * 70)
    print("TEST 3: Explicit variant filter (AFTERBLOW only)")
    print("=" * 70)
    results = search.search("pont", max_results=3, variant_filter="AFTERBLOW")
    ...
    for rule_id in variant_ids:
        ...
        if rule:
            variant = rule.get('variant', '(general)')
