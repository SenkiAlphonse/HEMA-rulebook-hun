...
class VariantAwareSearch:
    ...
    def search_for_competition(self, query: str, rule_variant: str = None, 
                               max_results: int = 5) -> List[Dict[str, Any]]:
        """
        Search rules for a specific rule variant
        
        Args:
            query: Search query
            rule_variant: Which variant this competition uses ("VOR", "COMBAT", "AFTERBLOW")
        ...
        for rule in self.rules:
            ...
            if rule_variant:
                ...
                if rule.get('variant') and rule.get('variant') != rule_variant:
                    continue
    ...
    def compare_variant(self, query: str) -> Dict[str, Any]:
        """
        Compare how a rule differs across variant options
        ...
        for variant in variants:
            comparison['variant'][variant] = self.search_for_competition(
...
def format_competition_results(results: List[Dict[str, Any]], variant: str = None) -> str:
        ...
        try:
            ...
            if cmd[0].lower() == 'search':
                ...
                if len(cmd) > 1:
                    ...
                    comp = search.compare_variant(query)
                    ...
                    for variant, rules in comp['variant'].items():
                        if rules:
